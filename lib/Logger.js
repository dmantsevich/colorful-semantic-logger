...
const { noop, convertToLevel, colorByType, color, LOG_LEVEL } = require('./utils');
...
class ColorfulSemanticLogger extends ChalkLogger {
  ...
  paintMsg(msg) {
    ...
    msg = (msg || '').replace(
      ...
      (pattern, type, value) => {
        if (tags[type]) {
          return this._paint(value, tags[type]);
        }
        return pattern;
      }
    );
  ...
  createNew(prefix, options) {
    return new this.constructor([prefix, this.name].join('.'), options);
  }
}
